module mines::move_feed_v12 {
    ...
    use mines::donations_v12;

    public entry fun tip_post(account: &signer, author: address, post_id: u64, amount: u64) acquires GlobalFeed {
        donations_v12::send_tip(account, author, amount);
        
        if (exists<GlobalFeed>(@mines)) {
            let feed = borrow_global_mut<GlobalFeed>(@mines);
            let len = vector::length(&feed.posts);
            
            // Optimistic check: if index == post_id
            if (post_id < len) {
                let post = vector::borrow_mut(&mut feed.posts, post_id);
                if (post.id == post_id) {
    ...
    public fun get_global_tip_stats(): (u64, address, u64) {
        if (exists<GlobalFeed>(@mines)) {
            donations_v12::get_global_tip_stats()
        } else {
            (0, @0x0, 0)
        }
    }
