module mines::move_feed_v14 {
    ...
    public entry fun tip_post(account: &signer, post_id: u64, amount: u64) acquires GlobalFeed {
        let feed = borrow_global_mut<GlobalFeed>(@mines);
        let i = 0;
        let len = vector::length(&feed.posts);
        while (i < len) {
            let post = vector::borrow_mut(&mut feed.posts, i);
            if (post.id == post_id) {
                // Transfer and record tip via Donations contract
                donations_v10::send_tip(account, post.author, amount);
